describe("Tambah kontak form kosong", () => {
  ...
  context("Uji komponen dan fungsi dengan kondisi tipe kontak kosong", () => {
    it('Mengirim semua required form kecuali tipe kontak dan grup kontak ekspekstasi tidak terkirim', () => {
      cy.get('#tipe_kontak').click();
      cy.contains('li', 'Pilih Tipe Kontak').click()
      
      cy.get('#sapaan').click()
      cy.contains('li', 'Bapak').click()
      cy.get('#nama').type('ryanandra')
      cy.get('#tipe_identitas').click()
      cy.contains('li', 'KTP').click()
      cy.get('#no_identitas').type('082131232133121')
      cy.get('[id="email.0"]').type('tap@gmail.com')
      cy.get('#nama_perusahaan').type('assist.id')
      cy.get('#no_hp').type('083173031348')
      cy.get('#no_telp').type('083173092445')
      cy.get('#no_fax').type('08123802173123321')
      cy.get('#no_npwp').type('0821321470214213')
      cy.get('#nitku').type('0832132131234214412')
      cy.get('[id=":rk:"]').type('Alamat penagihan')
      cy.get('[id=":rl:"]').type('Alamat Pengiriman')
      cy.get('[id="data_bank[0].bank_name"]').type('Bank 1')
      cy.get('[id="data_bank[0].bank_branch"]').type('Bank cabang 1')
      cy.get('[id="data_bank[0].holder_name"]').type('ryan 1')
      cy.get('[id="data_bank[0].rek_no"]').type('1')
