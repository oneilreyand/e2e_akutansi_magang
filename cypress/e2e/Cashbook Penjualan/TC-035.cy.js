describe('Penjualan - Pencarian Nama Pelanggan', () => {
  ...
  it('Sistem menampilkan data yang sesuai ketika mencari "inv/0018"', () => {
    // Input pencarian
    ...
    cy.get('.MuiTableBody-root .MuiTableRow-root').then(($rows) => {
      if ($rows.length > 0) {
        cy.log(`Ditemukan ${$rows.length} data untuk pencarian "inv/0018"`);
        
        // Cari di semua kolom yang mungkin mengandung nama pelanggan
        for (let col = 1; col <= 6; col++) {
          cy.get(`:nth-child(1) > :nth-child(${col})`).invoke('text').then((text) => {
            if (text.includes('inv/0018')) {
              cy.log(`✓ Kolom ${col} mengandung "inv/0018": ${text}`);
            }
          });
        }
      } else {
        cy.log('Tidak ada data yang ditemukan untuk pencarian "inv/0018"');
      }
    });
  });
