describe('Sistem menampilkan status Lunas pada tab Lunas', () => {
  beforeEach(() => {
    cy.request({
      method: 'POST',
  ...
  it('Sistem menampilkan status Lunas pada tab Lunas', () => {
    ...
    // Klik tab Lunas
    cy.get('.MuiTabs-list > :nth-child(4)').click();

    // Tunggu data reload setelah pindah tab
    cy.wait(2000);

    // Validasi tab Lunas aktif
    cy.get('.MuiTabs-list > .Mui-selected').should('contain.text', 'Lunas');

    // Validasi semua data memiliki status "Lunas"
    cy.get('tbody tr').should('exist').then(($rows) => {
      const rowCount = $rows.length;
      
      if (rowCount > 0) {
        cy.log(`Ditemukan ${rowCount} data dengan status Lunas`);
        
        // Validasi setiap baris satu per satu dengan query ulang
        for (let i = 0; i < rowCount; i++) {
          cy.get('tbody tr').eq(i).within(() => {
            cy.get('td').eq(4).should('contain.text', 'Lunas');
          });
        }
        
        cy.log('Semua data di tab Lunas memiliki status yang sesuai');
      } else {
        cy.log('Tidak ada data dengan status Lunas');
      }
    });
  });
});